...
2. Create env file:
   ...
   ```
3. Update `.env` values (especially `MONGO_URI`, `JWT_SECRET`, and AWS S3 vars).
4. Run server:
   ```bash
   npm run dev
   ```
...
## AWS Uploads For Course Materials
- Faculty course material endpoints support multipart uploads using the `file` field.
- Supported file types: PDF, PPT/PPTX, DOC/DOCX, MP4, WEBM, MOV, MKV.
- Uploaded files are stored in S3 and `fileUrl` is auto-generated by backend.
- You can still pass a direct `fileUrl` manually if no file is uploaded.
